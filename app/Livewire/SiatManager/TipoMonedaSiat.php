...
use App\Models\SiatDataTipoMoneda;
use App\Services\SiatService;
use DB;
use Log;

class TipoMonedaSiat extends BaseSiat
{
    public function render()
    {
        return view('livewire.siat-manager.tipo-moneda-siat');
    }

    public function loadData(){
        if (isset($this->siatSucursalPuntoVenta)) {
            $this->items = SiatDataTipoMoneda::where('siat_spv_id', $this->siatSucursalPuntoVenta->id)->get();
        }
    }
    ...
    public function getItems(SiatService $siatService){
        DB::beginTransaction();
        try {
            $tipoMonedas = $siatService->getTipoMonedas($this->siatProperty);
            
            SiatDataTipoMoneda::where('siat_spv_id', $this->siatSucursalPuntoVenta->id)->delete();

            $dataToInsert = [];
            $now = now(); // Para usar el mismo timestamp en todos los registros
            $tipoCatalogo = SiatDataTipoMoneda::$catalogoType; // Usar la propiedad estática
            ...
            if (!empty($dataToInsert)) {
                SiatDataTipoMoneda::insert($dataToInsert); 
            }
